// vocabularyData.js - GRE vocabulary with definitions and common word replacements
const greVocabulary = {
    // Group 1
    abound: {
        definition: "to exist in large numbers or quantities (verb)",
        replaces: ["flourish", "teem", "proliferate"],
    },
    didactic: {
        definition: "intended to teach or instruct; morally instructive (adjective)",
        replaces: ["educational", "instructive", "informative"],
    },
    benign: {
        definition: "gentle and kind; not harmful or malicious (adjective)",
        replaces: ["harmless", "gentle", "favorable"],
    },
    burgeon: {
        definition: "to grow or develop rapidly (verb)",
        replaces: ["flourish", "expand", "prosper"],
    },
    inimical: {
        definition: "hostile or unfriendly; adverse (adjective)",
        replaces: ["hostile", "adverse", "unfriendly"],
    },
    heterogeneous: {
        definition: "diverse in character or content; composed of different elements (adjective)",
        replaces: ["diverse", "varied", "mixed"],
    },
    empirical: {
        definition: "based on observation or experience rather than theory (adjective)",
        replaces: ["observed", "experimental", "practical"],
    },
    intransigent: {
        definition: "unwilling to compromise; stubborn (adjective)",
        replaces: ["stubborn", "unyielding", "adamant"],
    },
    innocuous: {
        definition: "not harmful or offensive; harmless (adjective)",
        replaces: ["harmless", "inoffensive", "safe"],
    },
    fecund: {
        definition: "intellectually productive; fertile (adjective)",
        replaces: ["fertile", "productive", "fruitful"],
    },
    furtive: {
        definition: "attempting to avoid notice or attention; secretive (adjective)",
        replaces: ["sneaky", "secretive", "sly"],
    },
    disseminate: {
        definition: "to spread widely; to diffuse (verb)",
        replaces: ["circulate", "distribute", "propagate"],
    },
    brazen: {
        definition: "bold and without shame; brassy (adjective)",
        replaces: ["bold", "shameless", "audacious"],
    },
    cacophonous: {
        definition: "involving or producing harsh, discordant sounds (adjective)",
        replaces: ["discordant", "grating", "jarring"],
    },
    intimate: {
        definition: "closely acquainted; private and personal (adjective)",
        replaces: ["close", "familiar", "personal"],
    },
    imperious: {
        definition: "arrogantly domineering or overbearing (adjective)",
        replaces: ["domineering", "overbearing", "authoritative"],
    },
    engender: {
        definition: "to cause or give rise to (verb)",
        replaces: ["generate", "produce", "create"],
    },
    intrepid: {
        definition: "fearless; adventurous (adjective)",
        replaces: ["fearless", "brave", "courageous"],
    },
    momentary: {
        definition: "lasting for a very short time; brief (adjective)",
        replaces: ["fleeting", "brief", "transient"],
    },
    feeble: {
        definition: "lacking physical strength; weak (adjective)",
        replaces: ["weak", "frail", "fragile"],
    },
    hamper: {
        definition: "to impede or hinder (verb)",
        replaces: ["hinder", "impede", "obstruct"],
    },
    feasible: {
        definition: "capable of being done or carried out (adjective)",
        replaces: ["practicable", "viable", "workable"],
    },
    calumny: {
        definition: "a false statement made to injure another's reputation (noun)",
        replaces: ["slander", "defamation", "libel"],
    },
    canonize: {
        definition: "to treat as sacred; to glorify (verb)",
        replaces: ["sanctify", "consecrate", "venerate"],
    },
    invigorate: {
        definition: "to give energy or strength to (verb)",
        replaces: ["energize", "revitalize", "stimulate"],
    },
    impertinent: {
        definition: "rude and not showing proper respect; irrelevant (adjective)",
        replaces: ["insolent", "impudent", "irrelevant"],
    },
    entitled: {
        definition: "believing oneself to be inherently deserving of privileges (adjective)",
        replaces: ["privileged", "arrogant", "presumptuous"],
    },
    irreverent: {
        definition: "showing a lack of respect for people or things that are generally respected (adjective)",
        replaces: ["disrespectful", "flippant", "impious"],
    },
    mundane: {
        definition: "lacking interest or excitement; commonplace (adjective)",
        replaces: ["ordinary", "commonplace", "prosaic"],
    },
    felicitous: {
        definition: "well-chosen or suited to the circumstances; apt (adjective)",
        replaces: ["apt", "appropriate", "fitting"],
    },
    indispensable: {
        definition: "absolutely necessary (adjective)",
        replaces: ["essential", "crucial", "vital"],
    },
    flout: {
        definition: "to openly disregard a rule or convention (verb)",
        replaces: ["defy", "disregard", "violate"],
    },
    candid: {
        definition: "truthful and straightforward; frank (adjective)",
        replaces: ["frank", "forthright", "blunt"],
    },
    censure: {
        definition: "to express severe disapproval of (verb)",
        replaces: ["condemn", "criticize", "reprimand"],
    },
    mitigate: {
        definition: "to make less severe or intense (verb)",
        replaces: ["alleviate", "lessen", "moderate"],
    },
    invasive: {
        definition: "tending to spread very quickly; intruding on others' privacy (adjective)",
        replaces: ["intrusive", "aggressive", "pervasive"],
    },
    pertinacious: {
        definition: "holding firmly to a belief or purpose; stubborn (adjective)",
        replaces: ["persistent", "tenacious", "determined"],
    },
    loathe: {
        definition: "to feel intense dislike or disgust for (verb)",
        replaces: ["despise", "detest", "abhor"],
    },
    nettlesome: {
        definition: "causing irritation or annoyance (adjective)",
        replaces: ["irritating", "annoying", "vexatious"],
    },
    forbear: {
        definition: "to refrain from doing something (verb)",
        replaces: ["refrain", "abstain", "desist"],
    },
    lament: {
        definition: "to express sorrow or regret for (verb)",
        replaces: ["mourn", "bemoan", "bewail"],
    },
    homogeneous: {
        definition: "of the same kind; uniform in structure (adjective)",
        replaces: ["uniform", "identical", "alike"],
    },
    castigate: {
        definition: "to criticize or punish severely (verb)",
        replaces: ["chastise", "rebuke", "reprimand"],
    },
    chicanery: {
        definition: "the use of trickery to achieve a political, financial, or legal purpose (noun)",
        replaces: ["deception", "trickery", "subterfuge"],
    },
    obsolete: {
        definition: "no longer in use or no longer useful (adjective)",
        replaces: ["outdated", "archaic", "antiquated"],
    },
    irresolute: {
        definition: "showing or feeling hesitancy; uncertain (adjective)",
        replaces: ["hesitant", "indecisive", "vacillating"],
    },
    presumptuous: {
        definition: "failing to observe the limits of what is permitted or appropriate (adjective)",
        replaces: ["arrogant", "overconfident", "presuming"],
    },
    malign: {
        definition: "to speak harmful untruths about; slander (verb)",
        replaces: ["defame", "slander", "vilify"],
    },
    nullify: {
        definition: "to make legally null and void; invalidate (verb)",
        replaces: ["invalidate", "annul", "revoke"],
    },
    haphazard: {
        definition: "lacking any obvious principle of organization (adjective)",
        replaces: ["random", "disorganized", "unsystematic"],
    },
    myopic: {
        definition: "lacking foresight or intellectual insight (adjective)",
        replaces: ["shortsighted", "narrow-minded", "unimaginative"],
    },
    humdrum: {
        definition: "lacking excitement; dull (adjective)",
        replaces: ["monotonous", "tedious", "dull"],
    },
    caustic: {
        definition: "sarcastic in a scathing and bitter way (adjective)",
        replaces: ["acerbic", "sarcastic", "mordant"],
    },
    coalesce: {
        definition: "to come together to form one mass or whole (verb)",
        replaces: ["merge", "unite", "amalgamate"],
    },
    opaque: {
        definition: "not able to be seen through; obscure (adjective)",
        replaces: ["cloudy", "murky", "impenetrable"],
    },
    laudable: {
        definition: "deserving praise and commendation (adjective)",
        replaces: ["commendable", "praiseworthy", "admirable"],
    },
    probity: {
        definition: "the quality of having strong moral principles; honesty and decency (noun)",
        replaces: ["integrity", "honesty", "uprightness"],
    },
    malleable: {
        definition: "easily influenced; pliable (adjective)",
        replaces: ["pliable", "adaptable", "flexible"],
    },
    obviate: {
        definition: "to remove a need or difficulty (verb)",
        replaces: ["preclude", "prevent", "eliminate"],
    },
    hodgepodge: {
        definition: "a confused mixture (noun)",
        replaces: ["jumble", "mishmash", "medley"],
    },
    nonchalant: {
        definition: "appearing casually calm and relaxed (adjective)",
        replaces: ["unconcerned", "indifferent", "detached"],
    },
    insipid: {
        definition: "lacking flavor; bland or uninteresting (adjective)",
        replaces: ["bland", "vapid", "tasteless"],
    },
    construe: {
        definition: "to interpret in a particular way (verb)",
        replaces: ["interpret", "understand", "infer"],
    },
    cogent: {
        definition: "clear, logical, and convincing (adjective)",
        replaces: ["convincing", "persuasive", "compelling"],
    },
    paradigmatic: {
        definition: "serving as a typical example of something (adjective)",
        replaces: ["archetypal", "quintessential", "representative"],
    },
    lax: {
        definition: "not sufficiently strict or careful (adjective)",
        replaces: ["negligent", "careless", "slack"],
    },
    proliferate: {
        definition: "to increase rapidly in number; multiply (verb)",
        replaces: ["multiply", "escalate", "snowball"],
    },
    neophyte: {
        definition: "a person who is new to a subject or activity (noun)",
        replaces: ["novice", "beginner", "tyro"],
    },
    omnipresent: {
        definition: "widely or constantly encountered (adjective)",
        replaces: ["ubiquitous", "pervasive", "universal"],
    },
    impede: {
        definition: "to delay or prevent by creating obstacles (verb)",
        replaces: ["hinder", "obstruct", "hamper"],
    },
    partial: {
        definition: "favoring one person or side over another; incomplete (adjective)",
        replaces: ["biased", "prejudiced", "incomplete"],
    },
    loquacious: {
        definition: "tending to talk a great deal; garrulous (adjective)",
        replaces: ["talkative", "voluble", "garrulous"],
    },
    contrite: {
        definition: "feeling or expressing remorse (adjective)",
        replaces: ["remorseful", "repentant", "penitent"],
    },
    compelling: {
        definition: "evoking interest or attention in a powerful way (adjective)",
        replaces: ["captivating", "gripping", "fascinating"],
    },
    pedantic: {
        definition: "overly concerned with formalism or precision (adjective)",
        replaces: ["fussy", "fastidious", "punctilious"],
    },
    marginalize: {
        definition: "to treat a person or group as insignificant (verb)",
        replaces: ["ostracize", "exclude", "sideline"],
    },
    specious: {
        definition: "superficially plausible but actually wrong (adjective)",
        replaces: ["misleading", "spurious", "fallacious"],
    },
    plastic: {
        definition: "capable of being shaped or molded (adjective)",
        replaces: ["malleable", "pliable", "moldable"],
    },
    oust: {
        definition: "to drive out or expel (verb)",
        replaces: ["expel", "eject", "evict"],
    },
    impetuous: {
        definition: "acting suddenly with little thought or care (adjective)",
        replaces: ["impulsive", "rash", "hasty"],
    },
    pensive: {
        definition: "engaged in deep or serious thought (adjective)",
        replaces: ["thoughtful", "contemplative", "reflective"],
    },
    misanthropic: {
        definition: "disliking humankind and avoiding human society (adjective)",
        replaces: ["antisocial", "cynical", "pessimistic"],
    },
    convoluted: {
        definition: "extremely complex and difficult to follow (adjective)",
        replaces: ["complicated", "intricate", "tangled"],
    },
    contend: {
        definition: "to assert something as a position in an argument; to compete (verb)",
        replaces: ["assert", "maintain", "compete"],
    },
    placid: {
        definition: "not easily upset or excited; calm and peaceful (adjective)",
        replaces: ["tranquil", "serene", "calm"],
    },
    panache: {
        definition: "flamboyant confidence of style or manner (noun)",
        replaces: ["flair", "style", "flamboyance"],
    },
    spurious: {
        definition: "not genuine, authentic, or true (adjective)",
        replaces: ["bogus", "false", "counterfeit"],
    },
    platitude: {
        definition: "a remark or statement that has been used too often to be interesting (noun)",
        replaces: ["cliché", "truism", "banality"],
    },
    palpable: {
        definition: "able to be touched or felt; obvious (adjective)",
        replaces: ["tangible", "perceptible", "noticeable"],
    },
    irascible: {
        definition: "having or showing a tendency to be easily angered (adjective)",
        replaces: ["irritable", "testy", "cranky"],
    },
    portend: {
        definition: "to be a sign or warning of (verb)",
        replaces: ["foreshadow", "presage", "augur"],
    },
    misnomer: {
        definition: "a wrong or inaccurate name or designation (noun)",
        replaces: ["misnaming", "mislabeling", "misapplication"],
    },
    covet: {
        definition: "to yearn to possess something belonging to another (verb)",
        replaces: ["desire", "crave", "hanker"],
    },
    copious: {
        definition: "abundant in supply or quantity (adjective)",
        replaces: ["abundant", "plentiful", "ample"],
    },
    polemical: {
        definition: "relating to or involving strongly critical or disputatious writing or speech (adjective)",
        replaces: ["controversial", "contentious", "argumentative"],
    },
    plodding: {
        definition: "slow-moving and unexciting (adjective)",
        replaces: ["laborious", "tedious", "monotonous"],
    },
    subjective: {
        definition: "based on personal feelings or opinions rather than facts (adjective)",
        replaces: ["personal", "individual", "biased"],
    },
    prescient: {
        definition: "having knowledge of events before they take place (adjective)",
        replaces: ["prophetic", "far-sighted", "visionary"],
    },
    perfidy: {
        definition: "deceitfulness; untrustworthiness (noun)",
        replaces: ["treachery", "betrayal", "deception"],
    },
    mercenary: {
        definition: "primarily concerned with making money at the expense of ethics (adjective)",
        replaces: ["grasping", "greedy", "venal"],
    },
    provincial: {
        definition: "having or showing the manners, viewpoints, etc., considered characteristic of unsophisticated inhabitants of a province (adjective)",
        replaces: ["parochial", "narrow-minded", "unsophisticated"],
    },
    negligent: {
        definition: "failing to take proper care over something (adjective)",
        replaces: ["careless", "remiss", "derelict"],
    },
    craven: {
        definition: "contemptibly lacking in courage; cowardly (adjective)",
        replaces: ["cowardly", "gutless", "spineless"],
    },
    cosmopolitan: {
        definition: "including people from many different countries; sophisticated (adjective)",
        replaces: ["sophisticated", "worldly", "cultured"],
    },
    precipitate: {
        definition: "to cause to happen suddenly or unexpectedly; hasty (verb)",
        replaces: ["precipitate", "hasten", "accelerate"],
    },
    prosaic: {
        definition: "lacking imagination; dull and commonplace (adjective)",
        replaces: ["unimaginative", "mundane", "pedestrian"],
    },
    subvert: {
        definition: "to undermine the power and authority of an established system (verb)",
        replaces: ["undermine", "destabilize", "sabotage"],
    },
    pristine: {
        definition: "in its original condition; unspoiled (adjective)",
        replaces: ["immaculate", "unspoiled", "untouched"],
    },
    profuse: {
        definition: "abundant or copious (adjective)",
        replaces: ["abundant", "copious", "lavish"],
    },
    meticulous: {
        definition: "showing great attention to detail; very careful and precise (adjective)",
        replaces: ["thorough", "painstaking", "scrupulous"],
    },
    rudimentary: {
        definition: "involving or limited to basic principles; elementary (adjective)",
        replaces: ["basic", "elementary", "fundamental"],
    },
    obsequious: {
        definition: "obedient or attentive to an excessive degree (adjective)",
        replaces: ["servile", "sycophantic", "fawning"],
    },
    decorum: {
        definition: "behavior in keeping with good taste and propriety (noun)",
        replaces: ["propriety", "etiquette", "decorousness"],
    },
    deference: {
        definition: "humble submission and respect (noun)",
        replaces: ["respect", "esteem", "reverence"],
    },
    profundity: {
        definition: "great depth of insight or knowledge (noun)",
        replaces: ["depth", "wisdom", "profundeness"],
    },
    remedial: {
        definition: "giving or intended as a remedy or cure (adjective)",
        replaces: ["therapeutic", "corrective", "curative"],
    },
    timorous: {
        definition: "showing or suffering from nervousness or fear (adjective)",
        replaces: ["timid", "fearful", "apprehensive"],
    },
    reproach: {
        definition: "to express disapproval or disappointment (verb)",
        replaces: ["rebuke", "reprimand", "chastise"],
    },
    pugnacious: {
        definition: "eager or quick to argue, quarrel, or fight (adjective)",
        replaces: ["combative", "belligerent", "contentious"],
    },
    mordant: {
        definition: "having or showing a sharp or critical quality (adjective)",
        replaces: ["caustic", "sarcastic", "acerbic"],
    },
    salutary: {
        definition: "producing good effects; beneficial (adjective)",
        replaces: ["beneficial", "advantageous", "wholesome"],
    },
    placate: {
        definition: "to make someone less angry or hostile (verb)",
        replaces: ["appease", "pacify", "mollify"],
    },
    deft: {
        definition: "demonstrating skill and cleverness (adjective)",
        replaces: ["adept", "dexterous", "skillful"],
    },
    desultory: {
        definition: "lacking a plan or purpose; occurring randomly (adjective)",
        replaces: ["random", "aimless", "haphazard"],
    },
    prophetic: {
        definition: "accurately predicting what will happen in the future (adjective)",
        replaces: ["predictive", "prescient", "oracular"],
    },
    restive: {
        definition: "impatient or uneasy under control (adjective)",
        replaces: ["restless", "fidgety", "agitated"],
    },
    // Group 2
    tortuous: {
        definition: "full of twists and turns; excessively complicated",
        replaces: ["winding", "complex", "difficult"],
    },
    robust: {
        definition: "strong and healthy; vigorous",
        replaces: ["strong", "solid", "tough"],
    },
    sagacious: {
        definition: "having or showing keen discernment and good judgment",
        replaces: ["wise", "smart", "clever"],
    },
    diffident: {
        definition: "modest or shy because of a lack of self-confidence",
        replaces: ["shy", "timid", "modest"],
    },
    prudent: {
        definition: "acting with or showing care and thought for the future",
        replaces: ["careful", "wise", "thoughtful"],
    },
    sporadic: {
        definition: "occurring at irregular intervals or only in a few places",
        replaces: ["scattered", "irregular", "occasional"],
    },
    tractable: {
        definition: "easily managed or controlled",
        replaces: ["manageable", "compliant", "docile"],
    },
    salubrious: {
        definition: "health-giving; healthy",
        replaces: ["healthy", "beneficial", "wholesome"],
    },
    sanguine: {
        definition: "optimistic or positive, especially in a difficult situation",
        replaces: ["hopeful", "upbeat", "optimistic"],
    },
    precarious: {
        definition: "not securely held or in position; dangerously likely to fall or collapse",
        replaces: ["risky", "unstable", "unsafe"],
    },
    slight: {
        definition: "small in degree; inconsiderable",
        replaces: ["small", "minor", "negligible"],
    },
    puerile: {
        definition: "childishly silly and immature",
        replaces: ["childish", "silly", "immature"],
    },
    derivative: {
        definition: "imitative of the work of another artist, writer, etc.",
        replaces: ["unoriginal", "copied", "imitative"],
    },
    dilatory: {
        definition: "slow to act; intended to cause delay",
        replaces: ["slow", "delayed", "tardy"],
    },
    punctilious: {
        definition: "showing great attention to detail or correct behavior",
        replaces: ["meticulous", "precise", "thorough"],
    },
    transient: {
        definition: "lasting only for a short time; impermanent",
        replaces: ["brief", "temporary", "fleeting"],
    },
    scant: {
        definition: "barely sufficient or adequate",
        replaces: ["minimal", "sparse", "insufficient"],
    },
    quirky: {
        definition: "having peculiar or unexpected traits",
        replaces: ["odd", "unusual", "eccentric"],
    },
    somnolent: {
        definition: "sleepy, drowsy",
        replaces: ["sleepy", "drowsy", "tired"],
    },
    quixotic: {
        definition: "exceedingly idealistic; unrealistic and impractical",
        replaces: ["idealistic", "impractical", "unrealistic"],
    },
    recondite: {
        definition: "difficult to understand; abstruse",
        replaces: ["obscure", "complex", "esoteric"],
    },
    ubiquitous: {
        definition: "present, appearing, or found everywhere",
        replaces: ["widespread", "universal", "omnipresent"],
    },
    sedulous: {
        definition: "showing dedication and diligence",
        replaces: ["diligent", "careful", "persistent"],
    },
    stoic: {
        definition: "enduring pain and hardship without showing feelings or complaining",
        replaces: ["impassive", "unemotional", "resigned"],
    },
    scrupulous: {
        definition: "diligent, thorough, and extremely attentive to details",
        replaces: ["meticulous", "careful", "thorough"],
    },
    soporific: {
        definition: "tending to induce drowsiness or sleep",
        replaces: ["boring", "sleep-inducing", "tedious"],
    },
    trivial: {
        definition: "of little value or importance",
        replaces: ["minor", "insignificant", "petty"],
    },
    taciturn: {
        definition: "reserved or uncommunicative in speech; saying little",
        replaces: ["quiet", "reserved", "silent"],
    },
    incredulous: {
        definition: "unwilling or unable to believe something",
        replaces: ["disbelieving", "skeptical", "doubtful"],
    },
    prodigal: {
        definition: "spending money or resources freely and recklessly; wastefully extravagant",
        replaces: ["wasteful", "extravagant", "spendthrift"],
    },
    tranquil: {
        definition: "free from disturbance; calm",
        replaces: ["calm", "peaceful", "serene"],
    },
    weary: {
        definition: "feeling or showing tiredness",
        replaces: ["tired", "exhausted", "fatigued"],
    },
    venal: {
        definition: "susceptible to bribery or corruption",
        replaces: ["corrupt", "dishonest", "mercenary"],
    },
    stern: {
        definition: "severe or strict in manner or appearance",
        replaces: ["strict", "harsh", "austere"],
    },
    utilitarian: {
        definition: "designed to be useful rather than attractive",
        replaces: ["practical", "functional", "plain"],
    },
    trifling: {
        definition: "unimportant or trivial",
        replaces: ["minor", "insignificant", "petty"],
    },
    wary: {
        definition: "feeling or showing caution about possible dangers or problems",
        replaces: ["cautious", "careful", "suspicious"],
    },
    ingenuous: {
        definition: "showing innocent or childlike simplicity and candidness",
        replaces: ["innocent", "naive", "artless"],
    },
    verbose: {
        definition: "using or containing more words than necessary",
        replaces: ["wordy", "long-winded", "prolix"],
    },
    zealous: {
        definition: "filled with eagerness and ardent interest in pursuit of something",
        replaces: ["eager", "enthusiastic", "fervent"],
    },
    tendentious: {
        definition: "expressing or promoting a particular, especially controversial, point of view",
        replaces: ["biased", "partisan", "prejudiced"],
    },
    vapid: {
        definition: "offering nothing that is stimulating or challenging; bland",
        replaces: ["dull", "insipid", "boring"],
    },
    turbulent: {
        definition: "characterized by conflict, disorder, or confusion; not stable or calm",
        replaces: ["chaotic", "tumultuous", "unstable"],
    },
    acrimonious: {
        definition: "bitter and sharp in language or tone",
        replaces: ["bitter", "hostile", "caustic"],
    },
    boisterous: {
        definition: "noisy, energetic, and cheerful",
        replaces: ["rowdy", "loud", "rambunctious"],
    },
    belligerent: {
        definition: "hostile and aggressive",
        replaces: ["aggressive", "hostile", "combative"],
    },
    altruistic: {
        definition: "showing a disinterested and selfless concern for the well-being of others",
        replaces: ["selfless", "charitable", "generous"],
    },
    adroit: {
        definition: "clever or skillful in using the hands or mind",
        replaces: ["skillful", "dexterous", "clever"],
    },
    accessible: {
        definition: "easily reached, entered, or used",
        replaces: ["reachable", "available", "approachable"],
    },
    chivalrous: {
        definition: "courteous and gallant, especially toward women",
        replaces: ["gallant", "courteous", "noble"],
    },
    beneficent: {
        definition: "doing or producing good, especially by charitable acts",
        replaces: ["charitable", "generous", "benevolent"],
    },
    baroque: {
        definition: "characterized by elaborate, ornate design; highly complex or intricate",
        replaces: ["ornate", "elaborate", "decorative"],
    },
    assertive: {
        definition: "having or showing a confident and forceful personality",
        replaces: ["confident", "bold", "decisive"],
    },
    acquisitive: {
        definition: "excessively interested in acquiring money or material things",
        replaces: ["greedy", "materialistic", "covetous"],
    },
    base: {
        definition: "without moral principles; ignoble",
        replaces: ["immoral", "contemptible", "dishonorable"],
    },
    churlish: {
        definition: "rude in a mean-spirited and surly way",
        replaces: ["rude", "boorish", "surly"],
    },
    canny: {
        definition: "having or showing shrewd judgment, especially in business matters",
        replaces: ["shrewd", "astute", "clever"],
    },
    byzantine: {
        definition: "excessively complicated, typically involving a great deal of administrative detail",
        replaces: ["complex", "convoluted", "labyrinthine"],
    },
    apt: {
        definition: "appropriate or suitable in the circumstances; quick to learn",
        replaces: ["suitable", "appropriate", "fitting"],
    },
    bogus: {
        definition: "not genuine or true; fake",
        replaces: ["fake", "false", "fraudulent"],
    },
    clandestine: {
        definition: "kept secret or done secretively, especially because illicit",
        replaces: ["secret", "covert", "furtive"],
    },
    cavalier: {
        definition: "showing a lack of proper concern; offhand",
        replaces: ["dismissive", "casual", "offhand"],
    },
    crestfallen: {
        definition: "sad and disappointed",
        replaces: ["dejected", "downcast", "disappointed"],
    },
    cataclysmic: {
        definition: "relating to or denoting a violent natural event",
        replaces: ["catastrophic", "disastrous", "devastating"],
    },
    duplicitous: {
        definition: "deceitful; characterized by duplicity",
        replaces: ["deceptive", "dishonest", "two-faced"],
    },
    complacent: {
        definition: "showing smug or uncritical satisfaction with oneself or one's achievements",
        replaces: ["smug", "self-satisfied", "conceited"],
    },
    distressed: {
        definition: "suffering from anxiety, sorrow, or pain",
        replaces: ["troubled", "anguished", "distraught"],
    },
    conciliatory: {
        definition: "intended or likely to placate or pacify",
        replaces: ["appeasing", "diplomatic", "pacifying"],
    },
    astringent: {
        definition: "causing contraction of body tissues, typically of the skin; harsh or severe",
        replaces: ["harsh", "severe", "caustic"],
    },
    cunning: {
        definition: "having or showing skill in achieving one's ends by deceit",
        replaces: ["crafty", "sly", "devious"],
    },
    outstrip: {
        definition: "to exceed or surpass",
        replaces: ["surpass", "exceed", "overtake"],
    },
    sever: {
        definition: "to divide by cutting or slicing; to end a relationship",
        replaces: ["cut", "disconnect", "separate"],
    },
    demur: {
        definition: "to raise objections or show reluctance",
        replaces: ["object", "hesitate", "protest"],
    },
    stigmatize: {
        definition: "to describe or identify in opprobrious terms",
        replaces: ["brand", "condemn", "denounce"],
    },
    sanction: {
        definition: "to authorize, approve, or allow; or a penalty for disobeying a law",
        replaces: ["authorize", "approve", "permit"],
    },
    desiccate: {
        definition: "to remove the moisture from; to make dry",
        replaces: ["dehydrate", "parch", "wither"],
    },
    equivocate: {
        definition: "to use ambiguous language to conceal the truth",
        replaces: ["prevaricate", "evade", "hedge"],
    },
    undermine: {
        definition: "to weaken gradually or insidiously",
        replaces: ["weaken", "sabotage", "erode"],
    },
    repudiate: {
        definition: "to refuse to accept; to reject",
        replaces: ["reject", "disown", "renounce"],
    },
    polarize: {
        definition: "to divide or cause to divide into two sharply contrasting groups",
        replaces: ["divide", "split", "separate"],
    },
    underscore: {
        definition: "to emphasize; to stress",
        replaces: ["emphasize", "highlight", "stress"],
    },
    supersede: {
        definition: "to take the place of; to supplant",
        replaces: ["replace", "supplant", "displace"],
    },
    vacillate: {
        definition: "to waver between different opinions or actions; be indecisive",
        replaces: ["waver", "fluctuate", "oscillate"],
    },
    venerate: {
        definition: "to regard with great respect or reverence",
        replaces: ["revere", "honor", "worship"],
    },
    wane: {
        definition: "to decrease in strength, intensity, or extent",
        replaces: ["diminish", "decline", "fade"],
    },
    abhor: {
        definition: "to regard with disgust and hatred",
        replaces: ["detest", "loathe", "despise"],
    },
    acquiesce: {
        definition: "to accept something reluctantly but without protest",
        replaces: ["consent", "submit", "yield"],
    },
    alienate: {
        definition: "to cause someone to become unfriendly or indifferent; estrange",
        replaces: ["estrange", "isolate", "antagonize"],
    },
    antedate: {
        definition: "to precede in time; come before",
        replaces: ["predate", "precede", "antedate"],
    },
    acclaim: {
        definition: "to praise enthusiastically and publicly",
        replaces: ["praise", "applaud", "commend"],
    },
    abet: {
        definition: "to encourage or assist someone to do something wrong",
        replaces: ["assist", "encourage", "support"],
    },
    banish: {
        definition: "to send someone away from a country or place as a punishment",
        replaces: ["exile", "expel", "ostracize"],
    },
    ascertain: {
        definition: "to find out or learn with certainty",
        replaces: ["determine", "discover", "establish"],
    },
    anoint: {
        definition: "to smear or rub with oil, especially as part of a religious ceremony",
        replaces: ["consecrate", "bless", "ordain"],
    },
    confound: {
        definition: "to cause surprise or confusion in someone; to mix up",
        replaces: ["confuse", "perplex", "bewilder"],
    },
    amend: {
        definition: "to change or modify for the better",
        replaces: ["revise", "modify", "correct"],
    },
    bridle: {
        definition: "to restrain or control; show resentment or anger",
        replaces: ["restrain", "control", "curb"],
    },
    comply: {
        definition: "to act in accordance with wishes, rules, or instructions",
        replaces: ["obey", "conform", "adhere"],
    },
    amalgamate: {
        definition: "to combine or unite to form one organization or structure",
        replaces: ["merge", "combine", "unite"],
    },
    cloak: {
        definition: "to hide or conceal something",
        replaces: ["conceal", "hide", "mask"],
    },
    attenuate: {
        definition: "to reduce the force, effect, or value of something",
        replaces: ["weaken", "diminish", "reduce"],
    },
    coin: {
        definition: "to invent or devise (a new word or phrase)",
        replaces: ["create", "invent", "devise"],
    },
    curtail: {
        definition: "to reduce in extent or quantity; impose a restriction on",
        replaces: ["reduce", "limit", "restrict"],
    },
    circumscribe: {
        definition: "to restrict or confine within certain limits",
        replaces: ["limit", "restrict", "confine"],
    },
    augment: {
        definition: "to make greater, more numerous, larger, or more intense",
        replaces: ["increase", "enhance", "expand"],
    },
    tout: {
        definition: "to attempt to sell or promote aggressively",
        replaces: ["promote", "advertise", "publicize"],
    },
    proclivity: {
        definition: "a natural inclination or tendency",
        replaces: ["tendency", "inclination", "propensity"],
    },
    spendthrift: {
        definition: "a person who spends money in an extravagant way",
        replaces: ["wastrel", "profligate", "squanderer"],
    },
    diatribe: {
        definition: "a forceful and bitter verbal attack against someone or something",
        replaces: ["tirade", "harangue", "invective"],
    },
    tact: {
        definition: "skill and sensitivity in dealing with others",
        replaces: ["diplomacy", "discretion", "sensitivity"],
    },
    skullduggery: {
        definition: "underhanded or unscrupulous behavior",
        replaces: ["trickery", "deceit", "chicanery"],
    },
    affinity: {
        definition: "a natural liking for or attraction to a person, thing, or idea",
        replaces: ["attraction", "kinship", "rapport"],
    },
    acolyte: {
        definition: "a devoted follower or attendant",
        replaces: ["follower", "disciple", "devotee"],
    },
    apathy: {
        definition: "lack of interest, enthusiasm, or concern",
        replaces: ["indifference", "lethargy", "disinterest"],
    },
    digression: {
        definition: "a temporary departure from the main subject in speech or writing",
        replaces: ["deviation", "tangent", "aside"],
    },
    animus: {
        definition: "strong hostility or ill feeling",
        replaces: ["hostility", "animosity", "resentment"],
    },
    coercion: {
        definition: "the practice of persuading someone to do something by using force or threats",
        replaces: ["compulsion", "intimidation", "duress"],
    },
    discrepancy: {
        definition: "an inconsistency between facts or claims",
        replaces: ["inconsistency", "contradiction", "variance"],
    },
    compromise: {
        definition: "an agreement reached by each side making concessions",
        replaces: ["agreement", "settlement", "concession"],
    },
    apologist: {
        definition: "a person who offers a defense of something controversial",
        replaces: ["defender", "advocate", "champion"],
    },
    consensus: {
        definition: "general agreement",
        replaces: ["agreement", "accord", "unanimity"],
    },
    utterly: {
        definition: "completely or absolutely",
        replaces: ["completely", "totally", "absolutely"],
    },
    apropos: {
        definition: "relevant or appropriate to what is being said or done",
        replaces: ["appropriately", "relevantly", "fittingly"],
    },
    abreast: {
        definition: "side by side and facing in the same direction; up to date with current information",
        replaces: ["alongside", "informed", "updated"],
    },
    // Group 3
    expedient: {
        definition:
            "convenient and practical although possibly improper or immoral",
        replaces: ["convenient", "useful"],
    },
    cumbersome: {
        definition: "large, heavy, and difficult to carry or use",
        replaces: ["heavy", "awkward"],
    },
    dwindling: {
        definition: "gradually diminishing in size, amount, or strength",
        replaces: ["decreasing", "shrinking"],
    },
    collaborate: {
        definition: "to work jointly on an activity or project",
        replaces: ["cooperate", "team up"],
    },
    distort: {
        definition: "to give a misleading or false account or impression of",
        replaces: ["twist", "change"],
    },
    elucidate: {
        definition: "to make clear; to explain",
        replaces: ["clarify", "explain"],
    },
    complementary: {
        definition:
            "combining in such a way as to enhance or emphasize each other's qualities",
        replaces: ["supporting", "matching"],
    },
    aversion: {
        definition: "a strong dislike or disinclination",
        replaces: ["dislike", "hatred"],
    },
    discomfit: {
        definition: "to make someone feel uneasy or embarrassed",
        replaces: ["embarrass", "upset"],
    },
    fabricate: {
        definition:
            "to invent or concoct (something), typically with deceitful intent",
        replaces: ["invent", "make up"],
    },
    debilitating: {
        definition: "making someone very weak and infirm",
        replaces: ["weakening", "harmful"],
    },
    eclipse: {
        definition:
            "to obscure or block out; surpass in importance or excellence",
        replaces: ["overshadow", "surpass"],
    },
    covert: {
        definition: "not openly acknowledged or displayed; secret",
        replaces: ["secret", "hidden"],
    },
    competent: {
        definition:
            "having the necessary ability, knowledge, or skill to do something successfully",
        replaces: ["capable", "skilled"],
    },
    divergent: {
        definition:
            "tending to be different or develop in different directions",
        replaces: ["different", "opposite"],
    },
    evade: {
        definition: "to escape or avoid, especially by cleverness or deceit",
        replaces: ["avoid", "escape"],
    },
    contentious: {
        definition: "causing or likely to cause an argument; controversial",
        replaces: ["controversial", "disputed"],
    },
    blithe: {
        definition: "showing a casual and cheerful indifference",
        replaces: ["cheerful", "carefree"],
    },
    dissent: {
        definition:
            "to hold or express opinions that are at variance with those previously held",
        replaces: ["disagree", "object"],
    },
    glum: {
        definition: "looking or feeling dejected; morose",
        replaces: ["gloomy", "sad"],
    },
    deliberate: {
        definition: "done consciously and intentionally; careful and unhurried",
        replaces: ["intentional", "careful"],
    },
    encyclopedic: {
        definition: "comprehensive in range of knowledge",
        replaces: ["complete", "thorough"],
    },
    credible: {
        definition: "able to be believed; convincing",
        replaces: ["believable", "reliable"],
    },
    correlate: {
        definition: "to have a mutual relationship or connection",
        replaces: ["connect", "relate"],
    },
    elated: {
        definition: "ecstatically happy",
        replaces: ["thrilled", "overjoyed"],
    },
    feckless: {
        definition:
            "lacking initiative or strength of character; irresponsible",
        replaces: ["weak", "useless"],
    },
    disingenuous: {
        definition:
            "not candid or sincere, typically by pretending that one knows less about something than one really does",
        replaces: ["dishonest", "fake"],
    },
    contempt: {
        definition:
            "the feeling that a person or a thing is beneath consideration or worthless",
        replaces: ["scorn", "disgust"],
    },
    distill: {
        definition:
            "to purify by condensation from a vapor; to extract the essential meaning or importance of",
        replaces: ["extract", "simplify"],
    },
    harbinger: {
        definition:
            "a person or thing that announces or signals the approach of another",
        replaces: ["sign", "warning"],
    },
    droll: {
        definition: "curious or unusual in a way that provokes dry amusement",
        replaces: ["amusing", "funny"],
    },
    exacerbate: {
        definition:
            "to make a problem, bad situation, or negative feeling worse",
        replaces: ["worsen", "increase"],
    },
    diffuse: {
        definition: "spread out over a large area; not concentrated; verbose",
        replaces: ["scattered", "widespread"],
    },
    deride: {
        definition: "to express contempt for; ridicule",
        replaces: ["mock", "tease"],
    },
    enchant: {
        definition: "to fill with delight; charm greatly",
        replaces: ["delight", "charm"],
    },
    fester: {
        definition:
            "(of a wound or sore) to become septic; to become worse or more intense",
        replaces: ["worsen", "rot"],
    },
    divulge: {
        definition: "to make known (private or sensitive information)",
        replaces: ["reveal", "tell"],
    },
    dawdle: {
        definition: "to waste time; be slow",
        replaces: ["delay", "waste time"],
    },
    dubious: {
        definition: "hesitating or doubting; not to be relied upon",
        replaces: ["doubtful", "uncertain"],
    },
    intrinsic: {
        definition: "belonging naturally; essential",
        replaces: ["natural", "basic"],
    },
    eccentric: {
        definition: "unconventional and slightly strange",
        replaces: ["unusual", "odd"],
    },
    exasperated: {
        definition: "intensely irritated and frustrated",
        replaces: ["frustrated", "annoyed"],
    },
    dictate: {
        definition:
            "to state or order authoritatively; to say or read aloud for another to transcribe",
        replaces: ["command", "order"],
    },
    entrenched: {
        definition: "firmly established and difficult to change",
        replaces: ["established", "fixed"],
    },
    iconoclastic: {
        definition:
            "attacking or ignoring cherished beliefs and long-held traditions",
        replaces: ["rebellious", "radical"],
    },
    dogmatic: {
        definition: "inclined to lay down principles as undeniably true",
        replaces: ["rigid", "stubborn"],
    },
    deflect: {
        definition:
            "to cause something to change direction; to divert the attention of",
        replaces: ["redirect", "turn away"],
    },
    ebullient: {
        definition: "cheerful and full of energy",
        replaces: ["enthusiastic", "energetic"],
    },
    // Group 4
    lucid: {
        definition: "expressed clearly; easy to understand",
        replaces: ["clear", "simple"],
    },
    inveterate: {
        definition: "having a particular habit, activity, or interest that is long-established",
        replaces: ["habitual", "constant"],
    },
    irksome: {
        definition: "irritating; annoying",
        replaces: ["annoying", "tiresome"],
    },
    munificent: {
        definition: "characterized by great generosity",
        replaces: ["generous"],
    },
    pious: {
        definition: "devoutly religious",
        replaces: ["devout", "religious"],
    },
    pervasive: {
        definition: "spreading widely throughout an area or a group of people",
        replaces: ["widespread", "common"],
    },
    metaphorical: {
        definition: "characteristic of or relating to metaphor; figurative",
        replaces: ["figurative", "symbolic"],
    },
    reticent: {
        definition: "not revealing one's thoughts or feelings readily",
        replaces: ["reserved", "quiet"],
    },
    pertinent: {
        definition: "relevant or applicable to a particular matter",
        replaces: ["relevant", "important"],
    },
    profligate: {
        definition: "recklessly extravagant or wasteful in the use of resources",
        replaces: ["wasteful", "excessive"],
    },
    miserly: {
        definition: "of or characteristic of a miser; very reluctant to spend money",
        replaces: ["stingy", "cheap"],
    },
    naive: {
        definition: "showing a lack of experience, wisdom, or judgment",
        replaces: ["innocent", "simple"],
    },
    providential: {
        definition: "occurring at a favorable time; fortunate",
        replaces: ["fortunate", "lucky"],
    },
    parsimonious: {
        definition: "unwilling to spend money or use resources; stingy",
        replaces: ["stingy", "frugal"],
    },
    stringent: {
        definition: "strict, precise, and exacting",
        replaces: ["strict", "tough"],
    },
    patent: {
        definition: "easily recognizable; obvious",
        replaces: ["obvious", "clear"],
    },
    modest: {
        definition: "unassuming or moderate in the estimation of one's abilities or achievements",
        replaces: ["humble", "simple"],
    },
    noble: {
        definition: "having or showing fine personal qualities or high moral principles",
        replaces: ["honorable", "good"],
    },
    peripheral: {
        definition: "relating to or situated on the edge or periphery of something",
        replaces: ["outer", "minor"],
    },
    numinous: {
        definition: "having a strong religious or spiritual quality; indicating the presence of divinity",
        replaces: ["spiritual", "holy"],
    },
    subservient: {
        definition: "prepared to obey others unquestioningly",
        replaces: ["obedient", "submissive"],
    },
    scintillating: {
        definition: "sparkling or shining brightly; brilliantly and excitingly clever or skillful",
        replaces: ["brilliant", "bright"],
    },
    petulant: {
        definition: "childishly sulky or bad-tempered",
        replaces: ["cranky", "grumpy"],
    },
    noxious: {
        definition: "harmful, poisonous, or very unpleasant",
        replaces: ["harmful", "toxic"],
    },
    parochial: {
        definition: "having a limited or narrow outlook or scope",
        replaces: ["narrow", "limited"],
    },
    rapacious: {
        definition: "aggressively greedy or grasping",
        replaces: ["greedy", "eager"],
    },
    provocative: {
        definition: "causing anger or another strong reaction, especially deliberately",
        replaces: ["bold"],
    },
    obscure: {
        definition: "not discovered or known about; uncertain",
        replaces: ["hidden", "unclear"],
    },
    surreptitious: {
        definition: "kept secret, especially because it would not be approved of",
        replaces: ["secret", "hidden"],
    },
    sensational: {
        definition: "causing great public interest and excitement",
        replaces: ["amazing", "stunning"],
    },
    pithy: {
        definition: "concise and forcefully expressive",
        replaces: ["brief", "short"],
    },
    pernicious: {
        definition: "having a harmful effect, especially in a gradual or subtle way",
        replaces: ["harmful", "damaging"],
    },
    pedestrian: {
        definition: "lacking inspiration or excitement; dull",
        replaces: ["dull", "boring"],
    },
    overt: {
        definition: "done or shown openly; plainly apparent",
        replaces: ["open", "public"],
    },
    tantalizing: {
        definition: "tormenting or teasing with the sight or promise of something that is unobtainable",
        replaces: ["tempting"],
    },
    sophisticated: {
        definition: "having, revealing, or proceeding from a great deal of worldly experience and knowledge",
        replaces: ["refined", "cultured"],
    },
    pliant: {
        definition: "easily bent; flexible",
        replaces: ["flexible", "soft"],
    },
    stalwart: {
        definition: "loyal, reliable, and hardworking",
        replaces: ["loyal", "steady"],
    },
    satirical: {
        definition: "using satire",
        replaces: ["mocking", "witty"],
    },
    tempestuous: {
        definition: "characterized by strong and turbulent or conflicting emotion",
        replaces: ["stormy", "wild"],
    },
    pellucid: {
        definition: "translucently clear; easily understood; lucid",
        replaces: ["clear", "simple"],
    },
    tantamount: {
        definition: "equivalent in seriousness to; virtually the same as",
        replaces: ["equal"],
    },
    skittish: {
        definition: "easily frightened or excited",
        replaces: ["nervous", "jumpy"],
    },
    sanctimonious: {
        definition: "making a show of being morally superior to other people",
        replaces: ["preachy", "smug"],
    },
    proficient: {
        definition: "competent or skilled in doing or using something",
        replaces: ["skilled", "good"],
    },
    prime: {
        definition: "of first importance; main",
        replaces: ["main", "chief"],
    },
    tenable: {
        definition: "able to be maintained or defended against attack or objection",
        replaces: ["valid", "logical"],
    },
    understated: {
        definition: "presented or expressed in a subtle and effective way",
        replaces: ["modest", "quiet"],
    },
    superfluous: {
        definition: "unnecessary, especially through being more than enough",
        replaces: ["extra", "needless"],
    },
    sound: {
        definition: "in good condition; not damaged, injured, or diseased",
        replaces: ["healthy", "strong"],
    },
    prolix: {
        definition: "using or containing too many words; tediously lengthy",
        replaces: ["wordy", "lengthy"],
    },
    radical: {
        definition: "(especially of change or action) relating to or affecting the fundamental nature of something",
        replaces: ["extreme", "drastic"],
    },
    terse: {
        definition: "sparing in the use of words; abrupt",
        replaces: ["brief", "short"],
    },
    rational: {
        definition: "based on or in accordance with reason or logic",
        replaces: ["logical", "reasonable"],
    },
    trenchant: {
        definition: "vigorous or incisive in expression or style",
        replaces: ["sharp", "keen"],
    },
    unscrupulous: {
        definition: "having or showing no moral principles; not honest or fair",
        replaces: ["dishonest", "corrupt"],
    },
    synoptic: {
        definition: "taking or involving a comprehensive mental view",
        replaces: ["broad", "general"],
    },
    recrudescent: {
        definition: "breaking out again or reappearing after a dormant period",
        replaces: ["renewed"],
    },
    sluggish: {
        definition: "slow-moving or inactive",
        replaces: ["slow", "lazy"],
    },
    urbane: {
        definition: "courteous and refined in manner",
        replaces: ["polite", "refined"],
    },
    scathing: {
        definition: "witheringly scornful; severely critical",
        replaces: ["harsh", "severe"],
    },
    thorough: {
        definition: "complete with regard to every detail; not superficial or partial",
        replaces: ["complete", "full"],
    },
    tepid: {
        definition: "showing little enthusiasm",
        replaces: ["cool", "mild"],
    },
    subordinate: {
        definition: "lower in rank or position",
        replaces: ["lower", "junior"],
    },
    temporal: {
        definition: "relating to worldly as opposed to spiritual affairs; secular",
        replaces: ["worldly", "earthly"],
    },
    universal: {
        definition: "relating to or done by all people or things in the world or in a particular group",
        replaces: ["general", "common"],
    },
    spartan: {
        definition: "showing or characterized by austerity or a lack of comfort or luxury",
        replaces: ["plain", "simple"],
    },
    subtle: {
        definition: "so delicate or precise as to be difficult to analyze or describe",
        replaces: ["delicate", "slight"],
    },
    versatile: {
        definition: "able to adapt or be adapted to many different functions or activities",
        replaces: ["flexible", "adaptable"],
    },
    virulent: {
        definition: "bitterly hostile or antagonistic",
        replaces: ["hostile", "bitter"],
    },
    visionary: {
        definition: "thinking about or planning the future with imagination or wisdom",
        replaces: ["creative", "inventive"],
    },
    unseemly: {
        definition: "(of behavior or actions) not proper or appropriate",
        replaces: ["improper", "wrong"],
    },
    transitory: {
        definition: "not permanent",
        replaces: ["brief", "passing"],
    },
    truculent: {
        definition: "eager or quick to argue or fight; aggressively defiant",
        replaces: ["angry", "hostile"],
    },
    superficial: {
        definition: "appearing to be true or real only until examined more closely",
        replaces: ["shallow", "simple"],
    },
    wayward: {
        definition: "difficult to control or predict because of unusual or perverse behavior",
        replaces: ["wild", "unruly"],
    },
    volatile: {
        definition: "liable to change rapidly and unpredictably, especially for the worse",
        replaces: ["unstable", "changeable"],
    },
    vociferous: {
        definition: "expressing feelings or opinions in a very loud or forceful way",
        replaces: ["loud", "noisy"],
    },
    veritable: {
        definition: "used as an intensifier, often to qualify a metaphor",
        replaces: ["real", "true"],
    },
    viable: {
        definition: "capable of working successfully; feasible",
        replaces: ["workable", "possible"],
    },
    woeful: {
        definition: "characterized by, expressive of, or causing sorrow or misery",
        replaces: ["sad", "awful"],
    },
    abject: {
        definition: "experienced or present to the maximum degree",
        replaces: ["total", "complete"],
    },
    arresting: {
        definition: "striking; eye-catching",
        replaces: ["striking", "stunning"],
    },
    allusive: {
        definition: "using or containing indirect references",
        replaces: ["indirect"],
    },
    adept: {
        definition: "very skilled or proficient at something",
        replaces: ["skilled", "expert"],
    },
    aberrant: {
        definition: "departing from an accepted standard",
        replaces: ["unusual", "strange"],
    },
    premeditate: {
        definition: "think out or plan beforehand",
        replaces: ["plan", "prepare"],
    },
    preclude: {
        definition: "prevent from happening; make impossible",
        replaces: ["prevent", "block"],
    },
    mimic: {
        definition: "imitate (someone or their actions or words)",
        replaces: ["copy", "imitate"],
    },
    reconcile: {
        definition: "restore friendly relations between",
        replaces: ["settle", "resolve"],
    },
    refine: {
        definition: "remove impurities or unwanted elements from; improve by making small changes",
        replaces: ["improve", "polish"],
    },
    relinquish: {
        definition: "voluntarily cease to keep or claim; give up",
        replaces: ["yield", "release"],
    },
    slander: {
        definition: "the action or crime of making a false spoken statement damaging to a person's reputation",
        replaces: ["insult"],
    },
    relish: {
        definition: "great enjoyment",
        replaces: ["enjoy", "savor"],
    },
    renounce: {
        definition: "formally declare one's abandonment of (a claim, right, or possession)",
        replaces: ["reject", "abandon"],
    },
    ruminate: {
        definition: "think deeply about something",
        replaces: ["think", "ponder"],
    },
    prevaricate: {
        definition: "speak or act in an evasive way",
        replaces: ["evade", "dodge"],
    },
    perpetuate: {
        definition: "make (something) continue indefinitely",
        replaces: ["continue", "maintain"],
    },
    skirt: {
        definition: "attempt to ignore, avoid, or get around (something problematic or difficult)",
        replaces: ["avoid", "bypass"],
    },
    tarnish: {
        definition: "make or become less valuable or respected",
        replaces: ["damage", "harm"],
    },
    upbraid: {
        definition: "find fault with (someone); scold",
        replaces: ["scold", "criticize"],
    },
    vanquish: {
        definition: "defeat thoroughly",
        replaces: ["defeat", "conquer"],
    },
    vitiate: {
        definition: "spoil or impair the quality or efficiency of",
        replaces: ["damage", "ruin"],
    },
    assail: {
        definition: "make a concerted or violent attack on",
        replaces: ["attack", "strike"],
    },
    antagonize: {
        definition: "cause (someone) to become hostile",
        replaces: ["anger", "upset"],
    },
    abscond: {
        definition: "leave hurriedly and secretly, typically to avoid detection or arrest",
        replaces: ["flee", "escape"],
    },
    supplicate: {
        definition: "ask or beg for something earnestly or humbly",
        replaces: ["beg", "plead"],
    },
    opprobrium: {
        definition: "harsh criticism or censure",
        replaces: ["criticism", "blame"],
    },
    manacle: {
        definition: "a metal band, chain, or shackle for fastening someone's hands or ankles",
        replaces: ["chain", "restraint"],
    },
    prowess: {
        definition: "skill or expertise in a particular activity or field",
        replaces: ["skill", "talent"],
    },
    predilection: {
        definition: "a preference or special liking for something; a bias in favor of something",
        replaces: ["preference", "liking"],
    },
    schism: {
        definition: "a split or division between strongly opposed sections or parties",
        replaces: ["split", "divide"],
    },
    predicament: {
        definition: "a difficult, unpleasant, or embarrassing situation",
        replaces: ["problem", "mess"],
    },
    strife: {
        definition: "angry or bitter disagreement over fundamental issues; conflict",
        replaces: ["conflict", "trouble"],
    },
    propriety: {
        definition: "conformity to conventionally accepted standards of behavior or morals",
        replaces: ["manners"],
    },
    sham: {
        definition: "a thing that is not what it is purported to be",
        replaces: ["fake", "trick"],
    },
    torpor: {
        definition: "a state of physical or mental inactivity; lethargy",
        replaces: ["stupor"],
    },
    transgression: {
        definition: "an act that goes against a law, rule, or code of conduct",
        replaces: ["offense", "crime"],
    },
    scorn: {
        definition: "the feeling or belief that someone or something is worthless or despicable",
        replaces: ["contempt", "mockery"],
    },
    tirade: {
        definition: "a long, angry speech of criticism or accusation",
        replaces: ["rant", "lecture"],
    },
    umbrage: {
        definition: "offense or annoyance",
        replaces: ["offense", "anger"],
    },
    veracity: {
        definition: "conformity to facts; accuracy",
        replaces: ["truth", "honesty"],
    },
    verisimilitude: {
        definition: "the appearance of being true or real",
        replaces: ["realism", "truth"],
    },
    vexation: {
        definition: "the state of being annoyed, frustrated, or worried",
        replaces: ["anger", "worry"],
    },
    apprehension: {
        definition: "anxiety or fear that something bad or unpleasant will happen",
        replaces: ["worry", "fear"],
    },
    abeyance: {
        definition: "a state of temporary disuse or suspension",
        replaces: ["pause"],
    },
    // Group 5
    amicable: {
        definition:
            "characterized by friendliness and absence of serious disagreement",
        replaces: ["friendly", "peaceful", "pleasant"],
    },
    chastise: {
        definition: "rebuke or reprimand severely",
        replaces: ["scold", "blame", "criticize"],
    },
    astute: {
        definition:
            "having or showing an ability to accurately assess situations or people",
        replaces: ["smart", "clever", "sharp"],
    },
    benevolent: {
        definition: "well meaning and kindly",
        replaces: ["kind", "caring", "generous"],
    },
    adverse: {
        definition: "preventing success or development; harmful; unfavorable",
        replaces: ["harmful", "negative", "bad"],
    },
    ardent: {
        definition: "enthusiastic or passionate",
        replaces: ["eager", "passionate", "intense"],
    },
    barren: {
        definition: "too poor to produce much or any vegetation",
        replaces: ["empty", "bare", "lifeless"],
    },
    abstract: {
        definition:
            "existing in thought or as an idea but not having a physical or concrete existence",
        replaces: ["vague", "unclear", "theoretical"],
    },
    apogee: {
        definition:
            "the highest point in the development of something; a climax or culmination",
        replaces: ["peak", "height", "summit"],
    },
    abide: {
        definition:
            "accept or act in accordance with (a rule, decision, or recommendation)",
        replaces: ["follow", "obey", "accept"],
    },
    animosity: {
        definition: "strong dislike or hostility",
        replaces: ["hatred", "hostility", "resentment"],
    },
    commence: {
        definition: "to begin or start",
        replaces: ["begin", "start", "open"],
    },
    berate: {
        definition: "to scold or criticize angrily",
        replaces: ["scold", "criticize", "blame"],
    },
    appropriate: {
        definition: "suitable or proper in the circumstances",
        replaces: ["proper", "suitable", "fitting"],
    },
    axiomatic: {
        definition: "self-evident or unquestionable",
        replaces: ["obvious", "clear", "certain"],
    },
    bombastic: {
        definition: "high-sounding but with little meaning; inflated",
        replaces: ["showy", "pompous", "wordy"],
    },
    affront: {
        definition: "an action or remark that causes outrage or offense",
        replaces: ["insult", "offense", "slight"],
    },
    aspersion: {
        definition: "an attack on the reputation or integrity of someone",
        replaces: ["insult", "slander", "attack"],
    },
    bravado: {
        definition: "a bold manner or false show of courage",
        replaces: ["swagger", "bluster", "pretense"],
    },
    aver: {
        definition: "to state or assert to be the case",
        replaces: ["claim", "state", "declare"],
    },
    convalescent: {
        definition: "recovering from illness or medical treatment",
        replaces: ["recovering", "healing", "improving"],
    },
    buoyant: {
        definition: "able to float; cheerful and optimistic",
        replaces: ["cheerful", "upbeat", "happy"],
    },
    archetype: {
        definition: "a very typical example of a certain person or thing",
        replaces: ["model", "example", "pattern"],
    },
    cease: {
        definition: "to come to an end or stop",
        replaces: ["stop", "end", "quit"],
    },
    cajole: {
        definition:
            "to persuade someone to do something by flattery or coaxing",
        replaces: ["coax", "persuade", "convince"],
    },
    agitate: {
        definition: "to stir up public interest or controversy",
        replaces: ["disturb", "upset", "trouble"],
    },
    bawdy: {
        definition:
            "dealing with sexual matters in a comical way; humorously indecent",
        replaces: ["crude", "vulgar", "indecent"],
    },
    callow: {
        definition: "young and inexperienced",
        replaces: ["inexperienced", "immature", "naive"],
    },
    barrage: {
        definition: "a concentrated delivery of artillery fire or criticism",
        replaces: ["flood", "stream", "bombardment"],
    },
    elementary: {
        definition:
            "relating to the most basic or essential aspects of a subject",
        replaces: ["basic", "simple", "fundamental"],
    },
    curb: {
        definition: "to restrain or keep in check",
        replaces: ["limit", "control", "restrain"],
    },
    buttress: {
        definition: "to support or strengthen",
        replaces: ["support", "strengthen", "reinforce"],
    },
    articulate: {
        definition:
            "having or showing the ability to speak fluently and coherently",
        replaces: ["clear", "eloquent", "fluent"],
    },
    conducive: {
        definition: "making a certain situation or outcome likely or possible",
        replaces: ["helpful", "favorable", "beneficial"],
    },
    chary: {
        definition: "cautiously or suspiciously reluctant to do something",
        replaces: ["cautious", "careful", "wary"],
    },
    august: {
        definition: "respected and impressive",
        replaces: ["grand", "impressive", "dignified"],
    },
    chagrin: {
        definition:
            "distress or embarrassment at having failed or been humiliated",
        replaces: ["embarrassment", "shame", "disappointment"],
    },
    capitulate: {
        definition: "to surrender or give in",
        replaces: ["surrender", "yield", "submit"],
    },
    cathartic: {
        definition:
            "providing psychological relief through expression of emotions",
        replaces: ["cleansing", "healing", "relieving"],
    },
    embellish: {
        definition: "to make more attractive by adding ornament or decoration",
        replaces: ["decorate", "enhance", "improve"],
    },
    decry: {
        definition: "to publicly denounce or condemn",
        replaces: ["condemn", "criticize", "denounce"],
    },
    condone: {
        definition: "to accept or allow behavior that is considered wrong",
        replaces: ["allow", "permit", "tolerate"],
    },
    auspicious: {
        definition: "conducive to success; favorable",
        replaces: ["favorable", "promising", "fortunate"],
    },
    corporeal: {
        definition: "relating to a physical body or material existence",
        replaces: ["physical", "bodily", "material"],
    },
    curmudgeon: {
        definition: "a bad-tempered or surly person",
        replaces: ["grouch", "complainer", "pessimist"],
    },
    burnish: {
        definition: "to polish or enhance the appearance of something",
        replaces: ["polish", "shine", "improve"],
    },
    collude: {
        definition:
            "to cooperate secretly or illegally in order to deceive others",
        replaces: ["conspire", "scheme", "cooperate"],
    },
    cogitate: {
        definition: "to think deeply about something",
        replaces: ["think", "ponder", "consider"],
    },
    decipher: {
        definition: "to convert into normal language or to understand",
        replaces: ["decode", "solve", "interpret"],
    },
    euphoric: {
        definition:
            "characterized by or feeling intense happiness or confidence",
        replaces: ["ecstatic", "joyful", "elated"],
    },
    duress: {
        definition:
            "threats, violence, constraints, or other action used to coerce someone",
        replaces: ["pressure", "force", "coercion"],
    },
    contravene: {
        definition: "to violate or go against a law, rule, or agreement",
        replaces: ["violate", "break", "disobey"],
    },
    bereft: {
        definition:
            "deprived of or lacking something, especially a non-material asset",
        replaces: ["lacking", "without", "deprived"],
    },
    doctrinaire: {
        definition:
            "seeking to impose a rigid doctrine without regard to practical considerations",
        replaces: ["rigid", "inflexible", "dogmatic"],
    },
    dirge: {
        definition: "a lament for the dead, especially a mournful song or poem",
        replaces: ["lament", "funeral song", "requiem"],
    },
    coy: {
        definition: "making a pretense of shyness or modesty",
        replaces: ["shy", "modest", "reserved"],
    },
    commiserate: {
        definition: "to express or feel sympathy or pity",
        replaces: ["sympathize", "console", "comfort"],
    },
    deportment: {
        definition: "a person's behavior or manners",
        replaces: ["behavior", "conduct", "manners"],
    },
    delusion: {
        definition:
            "a false belief or impression maintained despite being contradicted by reality",
        replaces: ["fantasy", "illusion", "misconception"],
    },
    exonerate: {
        definition: "to absolve someone from blame for a fault or wrongdoing",
        replaces: ["clear", "acquit", "vindicate"],
    },
    evoke: {
        definition:
            "to bring or recall a feeling, memory, or image to the mind",
        replaces: ["summon", "recall", "inspire"],
    },
    denounce: {
        definition: "to publicly declare to be wrong or evil",
        replaces: ["condemn", "criticize", "accuse"],
    },
    captious: {
        definition: "tending to find fault or raise petty objections",
        replaces: ["critical", "faultfinding", "picky"],
    },
    eclectic: {
        definition: "deriving ideas from a broad range of sources",
        replaces: ["varied", "diverse", "mixed"],
    },
    estimable: {
        definition: "worthy of respect or admiration",
        replaces: ["respectable", "admirable", "worthy"],
    },
    deprecate: {
        definition: "to express disapproval of something or someone",
        replaces: ["disapprove", "criticize", "condemn"],
    },
    conflagration: {
        definition:
            "an extensive fire that destroys a great deal of land or property",
        replaces: ["fire", "blaze", "inferno"],
    },
    extemporize: {
        definition: "to speak or perform without preparation",
        replaces: ["improvise", "ad-lib", "invent"],
    },
    dispense: {
        definition: "to distribute or provide a service or goods",
        replaces: ["distribute", "give", "provide"],
    },
    extrapolate: {
        definition:
            "to extend the application of a method or conclusion to an unknown situation",
        replaces: ["infer", "deduce", "conclude"],
    },
    fawn: {
        definition: "to display exaggerated flattery or affection",
        replaces: ["flatter", "grovel", "fuss"],
    },
    despotic: {
        definition: "relating to or characteristic of a despot; tyrannical",
        replaces: ["tyrannical", "oppressive", "authoritarian"],
    },
    conclusive: {
        definition: "serving to settle a matter or putting an end to doubt",
        replaces: ["final", "definitive", "decisive"],
    },
    equanimity: {
        definition: "mental calmness, composure, and evenness of temper",
        replaces: ["calmness", "composure", "serenity"],
    },
    euphemism: {
        definition:
            "a mild or indirect word substituted for one considered offensive",
        replaces: ["substitute", "replacement", "alternative"],
    },
    disdain: {
        definition:
            "the feeling that someone or something is unworthy of one's consideration",
        replaces: ["contempt", "scorn", "disgust"],
    },
    contretemps: {
        definition:
            "an unexpected and unfortunate occurrence; a minor dispute or disagreement",
        replaces: ["mishap", "setback", "incident"],
    },
    factious: {
        definition:
            "relating to or inclined to form factions or cause dissension",
        replaces: ["divisive", "quarrelsome", "troublesome"],
    },
    eloquent: {
        definition: "fluent or persuasive in speaking or writing",
        replaces: ["articulate", "expressive", "persuasive"],
    },
    falter: {
        definition: "to lose strength or momentum",
        replaces: ["stumble", "hesitate", "weaken"],
    },
    fret: {
        definition: "to be constantly or visibly worried or anxious",
        replaces: ["worry", "agonize", "stress"],
    },
    deviate: {
        definition: "to depart from an established course or standard",
        replaces: ["stray", "diverge", "wander"],
    },
    conspire: {
        definition:
            "to secretly plan with others to do something harmful or illegal",
        replaces: ["plot", "scheme", "plan"],
    },
    exorbitant: {
        definition: "exceeding the bounds of custom, propriety, or reason",
        replaces: ["excessive", "outrageous", "unreasonable"],
    },
    excoriate: {
        definition: "to criticize severely",
        replaces: ["criticize", "condemn", "attack"],
    },
    disperse: {
        definition: "to distribute or spread over a wide area",
        replaces: ["scatter", "spread", "distribute"],
    },
    conviction: {
        definition: "a firmly held belief or opinion",
        replaces: ["belief", "opinion", "certainty"],
    },
    fallow: {
        definition: "inactive or uncultivated",
        replaces: ["unused", "idle", "inactive"],
    },
    enthrall: {
        definition: "to capture the fascinated attention of",
        replaces: ["captivate", "fascinate", "enchant"],
    },
    fervent: {
        definition: "having or displaying a passionate intensity",
        replaces: ["passionate", "intense", "enthusiastic"],
    },
    glib: {
        definition: "fluent but insincere and shallow",
        replaces: ["smooth", "superficial", "insincere"],
    },
    disinterested: {
        definition: "not influenced by personal interest or gain",
        replaces: ["impartial", "neutral", "unbiased"],
    },
    delineate: {
        definition: "to describe or portray something precisely",
        replaces: ["describe", "outline", "define"],
    },
    fickle: {
        definition:
            "changing frequently, especially in affections or loyalties",
        replaces: ["changeable", "unreliable", "unpredictable"],
    },
    exigent: {
        definition: "pressing or demanding",
        replaces: ["urgent", "demanding", "critical"],
    },
    distend: {
        definition: "to swell or cause to swell by pressure from within",
        replaces: ["swell", "expand", "inflate"],
    },
    croon: {
        definition: "to sing or speak in a soft, soothing voice",
        replaces: ["sing", "hum", "whisper"],
    },
    feint: {
        definition: "a deceptive or pretended blow, thrust, or attack",
        replaces: ["fake", "trick", "deception"],
    },
    eradicate: {
        definition: "to destroy completely; to eliminate or get rid of",
        replaces: ["eliminate", "destroy", "remove"],
    },
    foment: {
        definition: "to instigate or stir up",
        replaces: ["incite", "provoke", "encourage"],
    },
    headstrong: {
        definition: "determined to do what one wants; stubborn",
        replaces: ["stubborn", "willful", "determined"],
    },
    escalate: {
        definition: "to increase in intensity, magnitude, or scope",
        replaces: ["increase", "intensify", "worsen"],
    },
    disentangle: {
        definition: "to free from entanglement or difficulty",
        replaces: ["untangle", "separate", "clarify"],
    },
    figurative: {
        definition:
            "using or containing figures of speech; metaphorical rather than literal",
        replaces: ["symbolic", "metaphorical", "nonliteral"],
    },
    haughty: {
        definition: "arrogantly superior and disdainful",
        replaces: ["arrogant", "proud", "snobbish"],
    },
    endemic: {
        definition:
            "native to or prevalent in a particular place or population",
        replaces: ["native", "local", "widespread"],
    },
    depose: {
        definition: "to remove from office or power, especially through force",
        replaces: ["remove", "overthrow", "oust"],
    },
    flagrant: {
        definition: "conspicuously offensive; blatant",
        replaces: ["blatant", "obvious", "outrageous"],
    },
    fledgling: {
        definition:
            "a person or organization that is immature, inexperienced, or underdeveloped",
        replaces: ["inexperienced", "new", "beginner"],
    },
    gaffe: {
        definition: "an unintentional act or remark causing embarrassment",
        replaces: ["mistake", "blunder", "error"],
    },
    intermittent: {
        definition:
            "occurring at irregular intervals; not continuous or steady",
        replaces: ["sporadic", "irregular", "occasional"],
    },
    exorcise: {
        definition: "to drive out an evil spirit; to free from evil influence",
        replaces: ["banish", "expel", "eliminate"],
    },
    exhort: {
        definition: "to strongly urge or encourage someone to do something",
        replaces: ["urge", "encourage", "persuade"],
    },
    flustered: {
        definition: "agitated or confused",
        replaces: ["confused", "agitated", "rattled"],
    },
    heady: {
        definition: "intoxicating or exhilarating",
        replaces: ["intoxicating", "thrilling", "exciting"],
    },
    enmity: {
        definition: "a state or feeling of active opposition or hostility",
        replaces: ["hostility", "hatred", "animosity"],
    },
    detente: {
        definition: "the easing of hostility or strained relations",
        replaces: ["truce", "relaxation", "improvement"],
    },
    gratuitous: {
        definition: "done without good reason; uncalled for",
        replaces: ["unnecessary", "unwarranted", "pointless"],
    },
    fortitude: {
        definition: "courage in pain or adversity",
        replaces: ["courage", "strength", "endurance"],
    },
    heterodox: {
        definition:
            "not conforming with accepted or orthodox standards or beliefs",
        replaces: ["unconventional", "unorthodox", "nonconformist"],
    },
    ire: {
        definition: "intense anger",
        replaces: ["anger", "rage", "fury"],
    },
    finicky: {
        definition: "excessively particular or fastidious",
        replaces: ["fussy", "picky", "particular"],
    },
    frailty: {
        definition: "the condition of being weak and delicate",
        replaces: ["weakness", "fragility", "vulnerability"],
    },
    gullible: {
        definition: "easily persuaded to believe something; credulous",
        replaces: ["naive", "trusting", "credulous"],
    },
    imperturbable: {
        definition: "unable to be upset or excited; calm",
        replaces: ["calm", "unflappable", "composed"],
    },
    // Group 6
    gauche: {
        definition: "lacking social grace or awareness",
        replaces: ["awkward", "clumsy"],
    },
    dowdy: {
        definition: "unfashionable and lacking style in appearance",
        replaces: ["plain", "dull"],
    },
    grovel: {
        definition:
            "to lie or crawl with the face downward in token of submission or humility",
        replaces: ["beg", "crawl"],
    },
    fortuitous: {
        definition: "happening by chance rather than intention",
        replaces: ["lucky", "random"],
    },
    histrionic: {
        definition: "overly theatrical or melodramatic in character or style",
        replaces: ["showy", "overdone"],
    },
    languid: {
        definition: "showing little or no spirit or animation; listless",
        replaces: ["slow", "tired", "weak"],
    },
    foil: {
        definition: "to prevent from succeeding; thwart",
        replaces: ["stop", "block", "halt"],
    },
    grievance: {
        definition:
            "a real or imagined wrong or other cause for complaint or protest",
        replaces: ["complaint", "issue", "wrong"],
    },
    idiosyncratic: {
        definition: "relating to idiosyncrasy; peculiar or individual",
        replaces: ["odd", "strange", "weird"],
    },
    implacable: {
        definition: "unable to be appeased or placated",
        replaces: ["harsh", "rigid", "strict"],
    },
    impudent: {
        definition: "not showing due respect",
        replaces: ["rude", "bold"],
    },
    echelon: {
        definition:
            "a level or rank in an organization, profession, or society",
        replaces: ["rank", "level", "tier"],
    },
    indecorous: {
        definition: "not in keeping with good taste and propriety; improper",
        replaces: ["rude", "crude", "improper"],
    },
    goad: {
        definition: "to provoke or urge someone to do something",
        replaces: ["push", "urge", "prod"],
    },
    implicit: {
        definition: "implied though not plainly expressed",
        replaces: ["hidden", "silent", "implied"],
    },
    lull: {
        definition: "a temporary interval of quiet or lack of activity",
        replaces: ["break", "pause", "rest"],
    },
    intertwined: {
        definition: "twisted or twined together",
        replaces: ["linked", "joined", "mixed"],
    },
    harangue: {
        definition: "a lengthy and aggressive speech",
        replaces: ["speech", "lecture", "talk"],
    },
    incidental: {
        definition: "happening as a minor accompaniment to something else",
        replaces: ["minor", "small"],
    },
    lambaste: {
        definition: "to criticize severely or harshly",
        replaces: ["scold", "attack", "blame"],
    },
    hysterical: {
        definition: "affected by or deriving from wildly uncontrolled emotion",
        replaces: ["wild", "crazy", "frantic"],
    },
    ennui: {
        definition:
            "a feeling of listlessness and dissatisfaction arising from a lack of occupation or excitement",
        replaces: ["boredom", "dullness"],
    },
    intrigue: {
        definition: "to arouse the curiosity or interest of; fascinate",
        replaces: ["interest", "attract", "draw"],
    },
    imminent: {
        definition: "about to happen; impending",
        replaces: ["coming", "near", "close"],
    },
    inviolate: {
        definition: "free from violation or desecration; untouched, intact",
        replaces: ["pure", "safe", "whole"],
    },
    mettlesome: {
        definition: "full of spirit and courage; spirited",
        replaces: ["brave", "bold", "lively"],
    },
    inundate: {
        definition: "to overwhelm with large amounts; flood",
        replaces: ["flood", "swamp", "drown"],
    },
    ploy: {
        definition: "a tactic or maneuver designed to gain advantage",
        replaces: ["trick", "plan", "scheme"],
    },
    ingrained: {
        definition: "firmly established; deeply rooted",
        replaces: ["deep", "fixed"],
    },
    miscreant: {
        definition: "a person who behaves badly or breaks the law",
        replaces: ["thief", "crook", "rogue"],
    },
    inchoate: {
        definition: "just begun and not fully formed; rudimentary",
        replaces: ["early", "rough"],
    },
    expatiate: {
        definition: "to speak or write at length or in detail",
        replaces: ["talk", "explain", "discuss"],
    },
    nominal: {
        definition: "existing in name only; minimal or insignificant",
        replaces: ["small", "slight", "tiny"],
    },
    incontrovertible: {
        definition: "not able to be denied or disputed; indisputable",
        replaces: ["certain", "clear"],
    },
    liability: {
        definition: "something that holds one back; a disadvantage",
        replaces: ["burden", "risk", "debt"],
    },
    mollify: {
        definition: "to calm or soothe; to appease",
        replaces: ["calm", "soothe", "ease"],
    },
    ironclad: {
        definition: "rigid, impregnable, unbreakable",
        replaces: ["solid", "strong", "firm"],
    },
    poise: {
        definition: "graceful composure or self-assurance",
        replaces: ["grace", "calm", "balance"],
    },
    insolent: {
        definition: "showing a rude and arrogant lack of respect",
        replaces: ["rude", "cocky"],
    },
    peccadillo: {
        definition: "a minor fault or sin; a slight offense",
        replaces: ["fault", "error", "mistake"],
    },
    penchant: {
        definition: "a strong inclination or liking for something",
        replaces: ["liking", "taste", "love"],
    },
    fraught: {
        definition:
            "filled with or destined to result in something undesirable",
        replaces: ["tense", "loaded"],
    },
    obdurate: {
        definition:
            "stubbornly refusing to change one's opinion or course of action",
        replaces: ["stubborn", "firm", "fixed"],
    },
    itinerant: {
        definition: "traveling from place to place",
        replaces: ["roving", "mobile"],
    },
    obstinate: {
        definition:
            "stubbornly refusing to change one's opinion or chosen course of action",
        replaces: ["stubborn", "rigid", "firm"],
    },
    neutralize: {
        definition: "to counteract or nullify the effect of something",
        replaces: ["counter", "cancel", "stop"],
    },
    jeopardize: {
        definition: "to put at risk; endanger",
        replaces: ["risk", "harm", "threaten"],
    },
    pomposity: {
        definition: "the quality of being pompous; self-importance",
        replaces: ["pride", "vanity", "arrogance"],
    },
    lampoon: {
        definition:
            "to publicly criticize someone or something by using ridicule or sarcasm",
        replaces: ["mock", "tease", "ridicule"],
    },
    philistine: {
        definition:
            "a person who is hostile or indifferent to culture and the arts",
        replaces: ["boor", "barbarian"],
    },
    quandary: {
        definition: "a state of perplexity or doubt",
        replaces: ["dilemma", "puzzle"],
    },
    fulcrum: {
        definition: "the point on which a lever rests or pivots",
        replaces: ["pivot", "hinge", "point"],
    },
    obstreperous: {
        definition: "noisy and difficult to control",
        replaces: ["loud", "rowdy", "wild"],
    },
    magnanimous: {
        definition:
            "generous or forgiving, especially toward enemies or rivals",
        replaces: ["kind", "generous", "fair"],
    },
    painstaking: {
        definition: "done with extreme care and thoroughness",
        replaces: ["careful", "precise"],
    },
    nonplussed: {
        definition:
            "so surprised or confused as to be at a loss as to what to think or do",
        replaces: ["shocked", "puzzled", "stumped"],
    },
    mercurial: {
        definition:
            "subject to sudden or unpredictable changes of mood or mind",
        replaces: ["moody", "fickle"],
    },
    proxy: {
        definition: "a person authorized to act on behalf of another",
        replaces: ["substitute", "deputy"],
    },
    lavish: {
        definition: "expending or giving profusely; generous",
        replaces: ["rich", "grand", "fancy"],
    },
    relegate: {
        definition: "to assign to an inferior position or place",
        replaces: ["demote", "lower", "banish"],
    },
    quarantine: {
        definition: "a state of isolation to prevent the spread of disease",
        replaces: ["isolation", "lockdown"],
    },
    imbroglio: {
        definition:
            "an extremely confused, complicated, or embarrassing situation",
        replaces: ["mess", "tangle"],
    },
    odious: {
        definition: "extremely unpleasant; repulsive",
        replaces: ["awful", "vile", "nasty"],
    },
    meritorious: {
        definition: "deserving reward or praise",
        replaces: ["worthy", "good"],
    },
    phlegmatic: {
        definition: "having or showing a slow and stolid temperament",
        replaces: ["calm", "cool", "dull"],
    },
    precipitous: {
        definition:
            "dangerously high or steep; done suddenly and without careful consideration",
        replaces: ["steep", "sharp", "sudden"],
    },
    oblivious: {
        definition:
            "not aware of or concerned about what is happening around one",
        replaces: ["unaware", "blind"],
    },
    relent: {
        definition:
            "to abandon or mitigate a harsh intention or cruel treatment",
        replaces: ["soften", "yield", "ease"],
    },
    lugubrious: {
        definition: "looking or sounding sad and dismal",
        replaces: ["sad", "gloomy", "dreary"],
    },
    repugnant: {
        definition: "extremely distasteful; unacceptable",
        replaces: ["awful", "gross", "foul"],
    },
    quash: {
        definition: "to reject or suppress completely",
        replaces: ["crush", "squash", "end"],
    },
    jocund: {
        definition: "cheerful and lighthearted",
        replaces: ["happy", "jolly", "glad"],
    },
    plucky: {
        definition:
            "having or showing determined courage in the face of difficulties",
        replaces: ["brave", "tough", "gutsy"],
    },
    mutiny: {
        definition: "an open rebellion against proper authority",
        replaces: ["uprising", "revolt", "riot"],
    },
    prodigious: {
        definition:
            "remarkably or impressively great in extent, size, or degree",
        replaces: ["huge", "vast", "great"],
    },
    pretentious: {
        definition:
            "attempting to impress by affecting greater importance or merit than is actually possessed",
        replaces: ["showy", "fancy", "phony"],
    },
    perpetrate: {
        definition:
            "to carry out or commit a harmful, illegal, or immoral action",
        replaces: ["commit", "perform"],
    },
    rhetoric: {
        definition:
            "language designed to have a persuasive or impressive effect, but often regarded as lacking in sincerity or meaningful content",
        replaces: ["talk", "speech", "words"],
    },
    macabre: {
        definition:
            "disturbing because concerned with or causing a fear of death",
        replaces: ["grim", "dark", "scary"],
    },
    sentimental: {
        definition:
            "prompted by or marked by sentiment, especially excessive or emotional feeling",
        replaces: ["mushy", "soft"],
    },
    quibble: {
        definition:
            "to argue about a trivial matter; to raise minor objections",
        replaces: ["argue", "nitpick"],
    },
    languish: {
        definition: "to lose or lack vitality; grow weak",
        replaces: ["weaken", "fade", "wilt"],
    },
    precocious: {
        definition:
            "having developed certain abilities or inclinations at an earlier age than usual",
        replaces: ["gifted", "advanced"],
    },
    paradoxical: {
        definition: "seemingly contradictory but nonetheless possibly true",
        replaces: ["odd", "strange", "weird"],
    },
    propensity: {
        definition: "a natural inclination or tendency",
        replaces: ["tendency", "habit"],
    },
    profound: {
        definition: "having great depth of meaning or feeling",
        replaces: ["deep", "serious"],
    },
    plaintive: {
        definition: "expressing sorrow; mournful or melancholy",
        replaces: ["sad", "wistful", "mournful"],
    },
    rigor: {
        definition: "the quality of being extremely thorough and careful",
        replaces: ["care", "strictness"],
    },
    morose: {
        definition: "sullen and ill-tempered",
        replaces: ["gloomy", "grumpy", "sad"],
    },
    squander: {
        definition:
            "to waste resources, especially money, extravagantly or foolishly",
        replaces: ["waste", "lose"],
    },
    ravage: {
        definition: "to cause severe damage to or destruction of",
        replaces: ["ruin", "wreck", "destroy"],
    },
    nadir: {
        definition: "the lowest point of anything",
        replaces: ["bottom", "low"],
    },
    remuneration: {
        definition: "payment for work or services",
        replaces: ["pay", "wage", "payment"],
    },
    perseverance: {
        definition:
            "persistence in doing something despite difficulty or delay in achieving success",
        replaces: ["grit", "persistence"],
    },
    qualm: {
        definition: "an uneasy feeling of doubt, worry, or fear",
        replaces: ["worry", "doubt", "fear"],
    },
    propagate: {
        definition: "to spread or promote an idea, theory, etc., widely",
        replaces: ["spread", "share", "extend"],
    },
    poignant: {
        definition: "evoking a keen sense of sadness or regret",
        replaces: ["sad", "moving", "touching"],
    },
    sparse: {
        definition: "thinly dispersed or scattered; not thick or dense",
        replaces: ["thin", "scant", "few"],
    },
    officious: {
        definition: "assertive of authority in an annoyingly domineering way",
        replaces: ["bossy", "pushy"],
    },
    swindle: {
        definition: "to obtain money or property by fraud or deceit",
        replaces: ["cheat", "trick", "fool"],
    },
    recant: {
        definition: "to withdraw or disavow a statement or belief publicly",
        replaces: ["retract", "deny", "reject"],
    },
    nimble: {
        definition: "quick and light in movement or action; agile",
        replaces: ["quick", "fast", "agile"],
    },
    slovenly: {
        definition: "messy and dirty; careless in appearance or work",
        replaces: ["messy", "sloppy", "dirty"],
    },
    render: {
        definition: "to provide or give something, especially a service",
        replaces: ["give", "provide", "supply"],
    },
    renege: {
        definition: "to go back on a promise, undertaking, or contract",
        replaces: ["quit", "retreat"],
    },
    recourse: {
        definition: "a source of help in a difficult situation",
        replaces: ["option", "choice", "help"],
    },
    quiescent: {
        definition: "in a state or period of inactivity or dormancy",
        replaces: ["quiet", "still", "calm"],
    },
    steadfast: {
        definition: "resolutely or dutifully firm and unwavering",
        replaces: ["loyal", "firm", "solid"],
    },
    ramification: {
        definition:
            "a consequence of an action or event, especially when complex or unwelcome",
        replaces: ["result", "effect", "outcome"],
    },
    tangible: {
        definition: "perceptible by touch; clear and definite; real",
        replaces: ["real", "solid", "actual"],
    },
    redoubtable: {
        definition: "formidable, especially as an opponent",
        replaces: ["feared", "tough", "mighty"],
    },
    ominous: {
        definition:
            "giving the impression that something bad is going to happen",
        replaces: ["scary", "grim", "threatening"],
    },
    soliloquy: {
        definition:
            "an act of speaking one's thoughts aloud when alone or regardless of hearers",
        replaces: ["speech", "monologue"],
    },
    repertoire: {
        definition:
            "a stock of skills, knowledge, or resources that a person possesses",
        replaces: ["range", "collection"],
    },
    stinting: {
        definition: "supplying or spending in an ungenerous or inadequate way",
        replaces: ["cheap", "tight", "mean"],
    },
    refute: {
        definition: "to prove a statement or theory to be wrong or false",
        replaces: ["disprove", "deny", "counter"],
    },
    reiterate: {
        definition: "to say something again or repeatedly",
        replaces: ["repeat", "restate", "echo"],
    },
    suspect: {
        definition: "to believe to be guilty, false, or undesirable",
        replaces: ["doubt", "distrust", "question"],
    },
    serene: {
        definition: "calm, peaceful, and untroubled",
        replaces: ["calm", "quiet", "peaceful"],
    },
    turpitude: {
        definition: "depraved or wicked behavior or character",
        replaces: ["evil", "wickedness", "vice"],
    },
    retiring: {
        definition: "shy and fond of being alone",
        replaces: ["shy", "quiet", "modest"],
    },
    outlandish: {
        definition: "looking or sounding bizarre or unfamiliar",
        replaces: ["weird", "crazy", "odd"],
    },
    spurn: {
        definition: "to reject with disdain or contempt",
        replaces: ["reject", "refuse", "snub"],
    },
    resilient: {
        definition:
            "able to withstand or recover quickly from difficult conditions",
        replaces: ["tough", "strong", "hardy"],
    },
    temper: {
        definition: "to moderate or mitigate something",
        replaces: ["soften", "reduce", "lessen"],
    },
    regress: {
        definition: "to return to a former or less developed state",
        replaces: ["decline", "slip", "worsen"],
    },
    // Group 7
    subside: {
        definition: "to become less intense, violent, or severe",
        replaces: ["ease", "wane", "decline"],
    },
    tedious: {
        definition: "too long, slow, or dull; tiresome or monotonous",
        replaces: ["dull", "slow", "boring"],
    },
    supplant: {
        definition: "to supersede or replace",
        replaces: ["replace", "oust"],
    },
    unalloyed: {
        definition: "complete and unreserved; not mixed with anything else",
        replaces: ["pure", "total", "complete"],
    },
    shrill: {
        definition: "high-pitched and piercing in sound",
        replaces: ["sharp", "piercing", "loud"],
    },
    propitious: {
        definition: "giving or indicating a good chance of success; favorable",
        replaces: ["good", "lucky", "favorable"],
    },
    stolid: {
        definition: "calm, dependable, and showing little emotion or animation",
        replaces: ["calm", "dull", "blank"],
    },
    resolute: {
        definition: "admirably purposeful, determined, and unwavering",
        replaces: ["firm", "steady", "strong"],
    },
    tentative: {
        definition: "not certain or fixed; provisional",
        replaces: ["unsure", "shaky", "uncertain"],
    },
    repercussion: {
        definition: "an unintended consequence of an action or event",
        replaces: ["result", "effect", "outcome"],
    },
    subsume: {
        definition: "to include or absorb in something else",
        replaces: ["include", "absorb"],
    },
    vitality: {
        definition: "the state of being strong and active; energy",
        replaces: ["energy", "strength", "life"],
    },
    tacit: {
        definition: "understood or implied without being stated",
        replaces: ["silent", "unspoken", "implied"],
    },
    undercut: {
        definition: "to offer goods or services at a lower price than a competitor",
        replaces: ["undersell"],
    },
    sophistry: {
        definition: "the use of clever but false arguments",
        replaces: ["trickery", "deceit"],
    },
    prurient: {
        definition: "having or encouraging an excessive interest in sexual matters",
        replaces: ["lewd", "dirty", "vulgar"],
    },
    temerity: {
        definition: "excessive confidence or boldness; audacity",
        replaces: ["nerve", "boldness"],
    },
    supple: {
        definition: "bending and moving easily and gracefully; flexible",
        replaces: ["flexible", "soft"],
    },
    unprecedented: {
        definition: "never done or known before",
        replaces: ["new", "rare", "unique"],
    },
    replenish: {
        definition: "to fill something up again",
        replaces: ["refill", "restock"],
    },
    surmount: {
        definition: "to overcome a difficulty or obstacle",
        replaces: ["overcome", "conquer"],
    },
    whimsical: {
        definition: "playfully quaint or fanciful",
        replaces: ["quirky", "odd", "playful"],
    },
    transcend: {
        definition: "to go beyond the range or limits of something",
        replaces: ["exceed", "surpass"],
    },
    wheedle: {
        definition: "to use flattery or coaxing to persuade someone",
        replaces: ["coax", "persuade"],
    },
    substantiate: {
        definition: "to provide evidence to support or prove the truth of something",
        replaces: ["prove", "confirm", "verify"],
    },
    sadistic: {
        definition: "deriving pleasure from inflicting pain",
        replaces: ["cruel", "mean", "vicious"],
    },
    tenuous: {
        definition: "very weak or slight",
        replaces: ["weak", "thin", "slight"],
    },
    valor: {
        definition: "great courage in the face of danger",
        replaces: ["bravery", "courage"],
    },
    vivacious: {
        definition: "attractively lively and animated",
        replaces: ["lively", "bright", "cheerful"],
    },
    vigilant: {
        definition: "keeping careful watch for possible danger or difficulties",
        replaces: ["watchful", "alert", "aware"],
    },
    tangential: {
        definition: "diverging from the main subject",
        replaces: ["unrelated", "irrelevant"],
    },
    treatise: {
        definition: "a written work dealing formally with a subject",
        replaces: ["book", "essay", "study"],
    },
    xenophobic: {
        definition: "showing dislike of foreigners",
        replaces: ["hateful", "biased", "prejudiced"],
    },
    wily: {
        definition: "skilled at gaining an advantage by deceit",
        replaces: ["sly", "tricky", "cunning"],
    },
    zenith: {
        definition: "the highest point or culmination",
        replaces: ["peak", "top", "summit"],
    },
    verve: {
        definition: "vigor and spirit or enthusiasm",
        replaces: ["spirit", "energy", "enthusiasm"],
    },
};