(function () {
    let isActive = true;
    let replacementPercentage = 25;
    let wordsReplacedCount = 0;
    let isProcessing = false;

    const storageAPI = getStorageAPI();
    storageAPI.local.get(
        ["greVocabActive", "replacementPercentage"],
        function (result) {
            isActive =
                result.greVocabActive !== undefined
                    ? result.greVocabActive
                    : true;
            replacementPercentage =
                result.replacementPercentage !== undefined
                    ? result.replacementPercentage
                    : 25;
    ...
    getRuntimeAPI().onMessage.addListener(function (
        request,
        sender,
        sendResponse
    ) {
            ...
            if (mutation.addedNodes.length > 0) {
                for (let i = 0; i < mutation.addedNodes.length; i++) {
                    ...
                    if (
                        ...
                        !node.classList?.contains("gre-popup")
                    ) {
                        shouldProcess = true;
                        break;
                    }
    ...
    function processNewContent() {
        ...
        const walker = document.createTreeWalker(
            ...
            NodeFilter.SHOW_TEXT,
            {
                acceptNode: function (node) {
                    ...
                    if (
                        node.parentNode.closest(".gre-processed") ||
        ...
        while ((currentNode = walker.nextNode())) {
            ...
            if (
                currentNode.textContent.trim().length < 4 ||
        ...
        if (textColor) {
            ...
            if (rgb && rgb.length >= 3) {
                ...
                if (textLuminance > 0.7) {
                    // If text is light, background is likely dark
                    return true;
                }
            }
        }
    ...
    function processPage() {
        ...
        document.querySelectorAll("p, div, span, article").forEach((el) => {
            if (
                !el.classList.contains("gre-popup") &&
        ...
        textNodes.forEach((textNode) => {
            ...
            for (let i = 0; i < parts.length; i++) {
                ...
                if (/\b\w{4,}\b/i.test(part)) {
                    ...
                    if (
                        replacementMap[lowerPart] &&
                        Math.random() * 100 <= replacementPercentage
                    ) {
                        ...
                        parts[
                            i
                        ] = `<span class="gre-word ${darkModeClass}" data-original="${part}" data-gre="${greWord}">${replacement}</span>`;
                        modified = true;
                        wordsReplacedCount++;
                    }
                }
            ...
            greVocabulary[greWord].replaces.forEach((commonWord, index) => {
                ...
                if (
                    ...
                    (replacementMap[lowerCommonWord] &&
                        greVocabulary[
                            replacementMap[lowerCommonWord]
                        ].replaces.indexOf(commonWord) > index)
                ) {
    ...
    document.addEventListener("click", function (e) {
        if (e.target.classList.contains("gre-word")) {
            ...
            !e.target.closest(".gre-popup") &&
            document.querySelector(".gre-popup")
        ) {
